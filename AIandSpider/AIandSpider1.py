...
def userLogin(musername):
    ...
    if (pwdresult):  # 如果找到了用户名对应的密码，
        ...
        count = 0
        while count < 2:
            if (tipsPawwword == pwdresult['tippassword']):
                # 对号成功，启动浏览器进行登录
        ...
        SmartTips(tipWord)
        time.sleep(0.2)
        LuYin("files/password.wav")  # 录音
        resultpassword = Identify("files/password.wav")  # 识别录音
        ...
        os.system('python2  D:\mPython2\spider\TaoBao\getGold.py' + musername + resultpassword)
...
if __name__ == "__main__":
    while True:
        ...
        #  调用百度接口,识别语音
        # 识别刚录好的本地文件
        theWord = Identify("files/luyin.wav")

        if theWord.find(u'金币') != -1 :
            print "ok"
            if theWord.isalnum(): # 包含了用户名
                print "ok1"
                # 正则匹配用户名
                pattern= re.compile('\d+')
                username = pattern.findall(theWord)[0]
                userLogin(username)
            else: # 没包含用户名
                # 提示用户告诉用户名
                print "ok2"
                mstr = "请输入用户名"
                SmartTips(mstr)
                LuYin("files/username1.wav")
                username = Identify("files/username1.wav")
                userLogin(username)
            ...
            os.system(u'python  D:\mPython2\AI\AIandSpider\JobpageWordCloud\mSalaryMain.py '+imgpath)

        else:
            print ("last")
            os.system(u"python D:\mPython2\AI\AIandSpider\Baidusearch.py" + theWord)
