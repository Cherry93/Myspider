    ...
    def process_request(self,request,spider):
        if  spider.name =="mytaobao": #指定仅仅处理这个名称的爬虫
            if request.url.find("login")!=-1:#判断是否登陆页面
                ...
                # spider.browser.switch_to.frame("iframe")
                username=spider.browser.find_element_by_id("username")
                password=spider.browser.find_element_by_id("password")
                username.clear()
                password.clear()
                time.sleep(3)
                username.send_keys("王湘英00") #账户
                time.sleep(5)
                password.send_keys("wxy15773978948@") #密码
                time.sleep(5)
                click=spider.browser.find_element_by_id("submit-btn")
                click.lick()
                time.sleep(8)
                print('ok')
